...
class CNNSentimentKim(minitorch.Module):
    ...
    def __init__(
        ...
        self.out = Linear(feature_map_size, 1)
    ...
    def forward(self, embeddings):
        ...
        """
        # TODO: Implement for Task 4.5.
        embeds = embeddings.permute(0, 2, 1)

        x1 = self.conv1.forward(embeds).relu()
        x2 = self.conv2.forward(embeds).relu()
        x3 = self.conv3.forward(embeds).relu()
        x = minitorch.nn.max(x1, 2) + minitorch.nn.max(x2, 2) + minitorch.nn.max(x3, 2)
        x = self.out(x.view(x.shape[0], x.shape[1]))
        if self.training:
            x = minitorch.nn.dropout(x, self.dropout)
        output = x.sigmoid().view(embeds.shape[0])
        return output
